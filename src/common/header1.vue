...
export default {
  ...
  methods: {
    ...
    handleIconClick(ev) {
      if (this.$route.path === '/queryAllResult') {
        ...
        // window.location.href =
        //   'http://47.105.75.254/#/queryAllResult?key=' + this.userinput
        this.$emit('sousuo', this.userinput)
      }
      this.$router.go(0)
    },
    // userinput(val) {
    //   if (val !== this.queryObj.key) {
    //     this.handleIconClick(val)
    //   }
    //   this.$router.go(0)
    // },
    handleSelect(item) {
  ...
  watch: {
    // this.queryObj.key//url上的关键字
    // this.userinput //输入框中关键字
    // this.handleIconClick()//查询新数据函数
  }
}
</script>
<style lang="scss">
